...
echo NETWORK_TEST
echo ============
printf "${green}PING DIRECT [ $test_site ]: ${reset}" && ping -q -c 1 $test_site | grep loss
printf "${green}PING DIRECT [ 8.8.8.8 ]: ${reset}" && ping -q -c 1 8.8.8.8 | grep loss
printf "${green}PING AWG10 [ $test_site ]: ${reset}" && ping -I awg10 -q -c 1 $test_site | grep loss
printf "${green}PING AWG10 [ 8.8.8.8 ]: ${reset}" && ping -I awg10 -q -c 1 8.8.8.8 | grep loss
echo
