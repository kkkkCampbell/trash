...
printf "${green}podkop [restart]: ${reset}" && service podkop restart >/dev/null 2>&1 && sleep 10 && echo
echo
...
printf "${green}DIRECT [ $test_site ]: ${reset}" && curl -s $test_site | head -c 50 && echo
