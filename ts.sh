...
echo NETWORK_TEST
echo ============
printf "${green}PING DIRECT: ${reset}" && ping -q -c 1 $test_site | grep loss
printf "${green}PING DIRECT: ${reset}" && ping -q -c 1 8.8.8.8 | grep loss
printf "${green}PING AWG10: ${reset}" && ping -I awg10 -q -c 1 $test_site | grep loss
printf "${green}PING AWG10: ${reset}" && ping -I awg10 -q -c 1 8.8.8.8 | grep loss
echo
